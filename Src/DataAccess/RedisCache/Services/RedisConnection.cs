...
namespace RedisCache.Services
{
    public class RedisConnection : IDisposable
    {
        ...
        private readonly TimeSpan ReconnectMinInterval = TimeSpan.FromSeconds(200);
        ...
        public async Task<T> BasicRetryAsync<T>(Func<IDatabase, Task<T>> func)
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex) when (ex is RedisConnectionException || ex is SocketException || ex is ObjectDisposedException || ex is RedisTimeoutException)
                {
                    reconnectRetry++;
